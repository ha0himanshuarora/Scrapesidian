...
function ExecutionViewer({ initialData }: { initialData?: ExecutionData }) {
    ...
    const query = useQuery({
        ...
        initialData: initialData ?? undefined, // Handle null case
        queryFn: () => {
            if (!initialData?.id) {
    ...
    return (
        <div className='flex w-full h-full'>
            {/* Sidebar */}
            <aside className='w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex-shrink-0 flex flex-col overflow-hidden'>
                {/* Status Label */}
                <ExecutionLabel icon={CircleDashedIcon} label="Status" value={<div className='font-semibold capaitalize flex gap-2 items-center'> <PhaseStatusBadge status={query.data?.status as ExecutionPhaseStatus}/><span>{query.data?.status}</span>
                </div>} />
                {/* Started At */}
...
function ExecutionLabel({icon,label,value}:{icon:LucideIcon,label:ReactNode,value:ReactNode}){
    ...
    return(
        ...
        <div className="text-muted-background flex items-center gap-2">
...
function ParameterViewer({title,subTitle,paramsJson}:{title:string,subTitle:string,paramsJson:string|null}){
    ...
    return <Card>
        ...
        <CardContent className='py-4'>
            <div className="flex flex-col gap-2">{(!params||Object.keys(params).length===0) &&(
...
function LogViewer({logs}:{logs:ExecutionLog[]|undefined}){
    ...
    return(
        <Card className='w-full'>
            <CardHeader className='rounder-lg rounded-b-none border-b py-4 bg-gray-50 dark:bg-background'>
