...
function TaskMenu() {
  return (<aside className='w-[340px] min-w-[340px] border-r-2 border-serarate h-full p-2 px-4 overflow-auto'>
...
function TaskMenuBtn({taskType}:{taskType:TaskType}){
    ...
    const onDragStart=(event:React.DragEvent,type:TaskType)=>{event.dataTransfer.setData("application/reactflow",type);
        event.dataTransfer.effectAllowed="move";
    };

    return <Button variant={"secondary"} className='flex justify-between items-center gap-2 w-full' draggable onDragStart={event=>onDragStart(event,taskType)}>
        <div className='flex gap-2'>
            <task.icon size={20}/>
            {task.label}
        </div>
        <Badge className='flex gap-2 items-center' variant={"outline"}><CoinsIcon size={16}/>{task.credits}</Badge>
    </Button>
}
