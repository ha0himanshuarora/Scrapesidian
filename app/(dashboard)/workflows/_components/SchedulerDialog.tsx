...
function SchedulerDialog(props:{cron:string|null; workflowId:string;}) {
    ...
    useEffect(()=>{
        ...
        } catch (error) {setValidCron(false);
        }
    },[cron]);
    ...
    <Dialog>
        <DialogTrigger asChild>
            <Button variant={"link"} size={"sm"} className={cn("text-sm p-0 h-auto text-orange-500",workflowHasValidCron && "text-primary")}>
                {workflowHasValidCron &&<div className='flex items-center gap-2'><ClockIcon/>{readableSavedCron}</div>}
        ...
        <DialogContent className='px-0'>
            ...
            <div className="p-6 space-y-4">
                <p className='text-muted-foreground text-sm'>
                    Specify a cron erpression to schedule periodic workflow execution.
                    All times are in UTC
                </p>
                <Input placeholder='E.g. * * * * *' value={cron} onChange={e=>setCron(e.target.value)}/>
                <div className={cn("bg-accent rounded-md p-4 border text-sm",validCron?"border-primary tet-primary":"border-destructive text-destructive")}>{validCron?readableCron:"Not a valid Cron expression"}</div>
                {workflowHasValidCron &&(
                    <DialogClose asChild>
                        <div className="">
