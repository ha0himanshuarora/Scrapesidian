...
function WorkflowCard({ workflow }: { workflow: Workflow }) {
  ...
  return (
    <Card className='border border-separater shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30 group/card'>
      <CardContent className='p-4 flex items-center justify-between h-[100px]'>
        <div className='flex items-center space-x-3'>
          ...
          {/* Workflow Name, Draft Tag, and Schedule Section */}
          <div className="text-base font-bold text-muted-foreground flex flex-col">
            <div className="flex items-center">
              <TooltipWrapper content={workflow.description}>
              <Link href={`/workflow/editor/${workflow.id}`} className="text-base font-bold hover:underline truncate">
                {workflow.name}
              </Link>
              </TooltipWrapper>
              {isDraft && (
